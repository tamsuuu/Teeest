...
import {
  ...
  conversationLimit15,
  mostUsedLanguage,
  myCompletedCoursesPaginate,
  myLearnings,
  myPosts,
  myProfilePage,
  overallStackleagueLevel,
  overallStackleagueStatistics,
  searchHistory,
...
export default function newsFeedPage(data) {
  group('My Profile', () => {
    ...
    group('My Profile apis', () => {
      ...
      stackleagueProfile(
        env.stackleague.request_url,
        data[Math.floor(Math.random() * data.length)].user.id,
        data[Math.floor(Math.random() * data.length)].jwt,
        data[Math.floor(Math.random() * data.length)].user.username
      );
      ...
      // overall stackleague statistics
      overallStackleagueStatistics(
        env.stacklab_exam.request_url,
        data[Math.floor(Math.random() * data.length)].jwt,
        data[Math.floor(Math.random() * data.length)].user.username
