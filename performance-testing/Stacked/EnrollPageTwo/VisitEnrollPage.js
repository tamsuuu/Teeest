...
export default function VisitMarketPlaceEnrollPage(data) {
  ...
  group('Marketplace Course View', () => {
    group('Course View Page', () => {
      let visitPage = http.get(`${env.stacked.url}/marketplace/nbmYkgdqYDaq`, {
        headers: {
      ...
      let pageRender = check(visitPage, {
        'Response is 200': (r) => r.status === 200,
      });
    });

    group('Course View API', () => {
      // verify
      verifyToken(
    ...
    group('Enroll Action', () => {
      ...
      enrollCourse(
        env.stacked.request_url,
        env.stacked.coursePurchasePayload,
        data[Math.floor(Math.random() * data.length)].jwt,
        data[Math.floor(Math.random() * data.length)].user.username
      );
    });
